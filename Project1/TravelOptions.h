...
class TravelOptions
{
   ...
   */
    ...
    */
    bool is_sorted() const
    {
        ...
        while (temp1 != nullptr && temp1->next != nullptr){
            ...
            temp2 = temp1->next;
            if (temp2->price < temp1->price)
                return false;
            else if (temp1->price == temp2->price)
                if (temp2->time < temp1->time)
                    return false;
    ...
    */
    bool is_pareto() const
    {
        ...
        while (temp1 != nullptr){
            temp2 = temp1->next;  
            while (temp2 != nullptr){
                if (compare(temp1, temp2) != incomparable)
                    return false;
                temp2 = temp2->next;
            }
            temp1 = temp1->next;  
    ...
    */
    bool is_pareto_sorted() const
    {
        ...
        while (temp1 != nullptr && temp1->next != nullptr){
            temp2 = temp1->next;
            if (temp2->price <= temp1->price || temp2->time >= temp1->time)
                return false;
    ...
    bool insert_sorted(double price, double time) 
    {
        ...
        else if(temp->price > price || (temp->price == price && temp->time >= time)){
            front = newNode;
            front->next = temp;
        }
        //insert in middle of list
            ...
            tempNext = temp->next;   
                //if price is equal                        
                if (price == temp->price && (price < tempNext->price || s(price == tempNext->price && time <= tempNext->time))){
                    temp->next = newNode;
                    newNode->next = tempNext;
                    return true;
                }
                //if price is greater
                else if (price > temp->price && (price < tempNext->price || (price == tempNext->price && time <= tempNext->time))){
                            temp->next = newNode;
                            newNode->next = tempNext;
                            return true;
                        }
    ...
    //helper function to delete given pair from list
    void removePair(Node* prev, Node* curr){
        //pair at front
        if (curr == front){
            front = front->next;
            delete curr;
    ...
    bool insert_pareto_sorted(double price, double time)
    {
        ...
        //check if any pairs in the list are duplicates of or dominate the new pair
        while (curr != nullptr){
            if (compare(curr->price, curr->time, price, time) == better ||
                compare(curr->price, curr->time, price, time) == equal){
                return true;
            }
            curr = curr->next;
        }

        //set curr back to front, to iterate through again
        curr = front;

        //remove any pairs in the list that are dominated by the new pair
        while (curr != nullptr){
            if (compare(curr->price, curr->time, price, time) == worse){
                removePair(prev, curr);
                if (prev != nullptr)
                    curr = prev;
            }
            prev = curr;
        ...
        //finally, insert the pair into the list, 
        //maintaining the sorted and pareto property of the list
   ...
   */
    ...
    //helper function that adds a new node after "tail"
    Node* addPair(Node* tail, double price, double time){
        Node* newNode = new Node(price, time, nullptr);
    ...
    TravelOptions *union_pareto_sorted(const TravelOptions &other) const
    {
        ...
        {
            printf("union_pareto_sorted() preconditions not met\n");
            return nullptr;
        }

        TravelOptions* newOptions = new TravelOptions;

        Node* temp1 = front;
        Node* temp2 = other.front;
        Node* tail = newOptions->front;

        //merge the lists together to be one big sorted list (not pareto yet!)
        while(temp1 != nullptr && temp2 != nullptr){
            //case price1 < price2
            if (temp1->price < temp2->price){
                tail = newOptions->addPair(tail, temp1->price, temp1->time);
                temp1 = temp1->next;
            }
            //case price1 = price2
            else if (temp1->price == temp2->price){
                //subcase time1 <= time2
                if (temp1->time <= temp2->price){
                    tail = newOptions->addPair(tail, temp1->price, temp1->time);
                    temp1 = temp1->next;
                }
                //subcase time1 > time2
                else{
                    tail = newOptions->addPair(tail, temp2->price, temp2->time);
                    temp2 = temp2->next;
                }
            }
            //case price1 > price2
        ...
        //tack on the rest of the non-empty list
        if (temp1 == nullptr){
            while (temp2 != nullptr){
                tail = newOptions->addPair(tail, temp2->price, temp2->time);
                temp2 = temp2->next;
        ...
        if (newOptions->prune_sorted())
            printf("prune_sorted the newOptions list\n");
   ...
   */
    bool prune_sorted()
    {
        ...
        while (curr != nullptr){
            if (prev->price >= curr->price || prev->time <= curr->time){
                removePair(prev, curr);
                curr = prev;
            }
            prev = curr;
        ...
        if (is_pareto_sorted())
            printf("list is now pareto/sorted!\n");
        return true;
    }
   ...
   */
    TravelOptions *join_plus_plus(const TravelOptions &other) const
    {
        ...
        //check if one or more lists are empty
        ...
        //calls insert_pareto_sorted() with all combinations of pairs
        while (temp1 != nullptr){
            temp2 = other.front;
            while (temp2 != nullptr){
                newOptions->insert_pareto_sorted(temp1->price + temp2-> price, temp1->time + temp2->time);
                temp2 = temp2->next;
            }
            temp1 = temp1->next;
        }
   ...
   */
    static double MAX(double time1, double time2){
        if (time1 >= time2)
            return time1;
        else 
            return time2;
    ...
    TravelOptions *join_plus_max(const TravelOptions &other) const
    {
        ...
        //tack on next best option continuously until either of the lists is depleted
        while (temp1 != nullptr && temp2 != nullptr){
            max = MAX(temp1->time, temp2->time);
            tail = newOptions->addPair(tail, temp1->price + temp2->price, max);
            if (max == temp1->time)
                temp1 = temp1->next;
            else
                temp2 = temp2->next;  
        ...
        if (newOptions->is_pareto_sorted()){
            printf("new list is pareto/sorted\n");
        }
   ...
   */
    TravelOptions *split_sorted_pareto(double max_price)
    {
        ...
        //traverse until last option within the budget is found
        while (temp->next != nullptr && temp->next->price <= max_price)
            temp = temp->next;
        
        //point newOptions front at first over-budget options
        newOptions->front = temp->next;
        
        //point last within_budget option's next at nullptr        
        temp->next = nullptr;
