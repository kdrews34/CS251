...
public:
  ...
  bool dag_num_paths(vector<vertex_label> &rpt)
  {
    ...
    // do bfs like search from each input vertex, everytime that search comes across a vertex,increment pred (num input paths) 
    ...
    //then do a bfs like search from each vertex, multiplying report[src].dist (num output paths = 1) by the number of outgoing edges
    ...
    for (int i = 0; i < vertices.size(); i++){
      ...
      while (!q.empty()){
        ...
        for (edge &e: vertices[u].outgoing){
          v = e.vertex_id;
          rpt[src].dist *= vertices[u].outgoing.size();
          q.push(v);
        } // end for
      } // end while
    } // end for
    ...
    for (int i = 0; i < vertices.size(); i++){
      printf("%d num paths = %d * %d\n", i, rpt[i].pred, rpt[i].dist);
      rpt[i].npaths = rpt[i].pred * rpt[i].dist;
    }
