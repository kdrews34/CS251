  ...
  bool bfs(int src, std::vector<vertex_label> &report)
  {
    ...
    while (!q.empty())
    {
      ...
      for (edge &e : vertices[u].outgoing)
      {
        ...
        {
          report[v].dist = report[u].dist + 1; // initialize shortest path
          report[v].pred = u;
          report[v].state = DISCOVERED;
          report[v].npaths = 0; // initialize first shortest path
          // enqueue newly discovered vertex
          q.push(v);
        }

        if (report[v].dist == report[u].dist + 1){ // is this another shortest path of the same length?
          printf("found another shortest path for %d...incrementing dist[%d] to %d\n", v, v, report[v].npaths + 1);
          report[v].npaths += report[u].npaths; // increase number of shortest paths
        }
...
public:
                    ...
                    int dest, vector<int> &path)
  {
    ...
    // start from rpt sub dest, pushing dest, then pushing each pred onto the path vector
    vector<int> tempPath;
    int tempVertex = dest;

    while (tempVertex != rpt[tempVertex].pred && rpt[tempVertex].pred != -1)
    { // push dest and all predecessors to form path in reverse
      tempPath.push_back(tempVertex);
      tempVertex = rpt[tempVertex].pred;
    }
    if (rpt[tempVertex].pred != -1)
      tempPath.push_back(tempVertex); //push the source vertex on last

    for (int i = tempPath.size() - 1; i >= 0; i--) // flip the path around so it is in the correct order
...
bool dag_critical_paths(vector<vertex_label> &rpt)
  {
    ...
    for (int i = 0; i < vertices.size(); i++){ // get source vertex
      if (vertices[i].incoming.size() == 0)
      {
        ...
        rpt[src].dist = 0;
        rpt[src].pred = src;
        indeg = vertices[i].incoming.size();
  ...
  bool dag_num_paths(vector<vertex_label> &rpt)
  {
    ...
    int u, v, src;
    vector<int> inPaths;
    for (int i = 0; i < rpt.size(); i++) // init inPaths
      inPaths.push_back(0);

    vector<int> outPaths;
    for (int i = 0; i < rpt.size(); i++){ // init outPaths
      outPaths.push_back(1);
      outPaths[i] = 0;
    }
    ...
    // do bfs like search from each input vertex, everytime that search comes across a vertex,increment num input paths
    ...
    //then do a bfs like search from each vertex, multiplying num output paths by the number of outgoing edges
    for (int i = 0; i < vertices.size(); i++){
      //indeg = vertices[i].incoming.size();
      //indegrees.push_back(indeg);
      //if (indeg == 0)
    ...
    // then finally go through each vertex again to calculate npaths = input_paths + output_paths
    for (int i = 0; i < vertices.size(); i++){
      rpt[i].npaths = inPaths[i] * outPaths[i];
    }
  ...
  bool valid_topo_order(const vector<int> &order)
  {
    ...
    vector<int> inDegree; // holds initial indegree of each vertex

    for (int i = 0; i < vertices.size(); i++){ // setting up inDegree vector
      inDegree.push_back(0);
    }

    for (int i = 0; i < vertices.size(); i++){ // initialize indegrees of each vertex
      for (edge &e: vertices[i].outgoing){
        inDegree[e.vertex_id]++;
      }
    }

    for (int i = 0; i < order.size(); i++){ // go through each vertex in the order parameter
      if (inDegree[order[i]] > 0){ // if the vertex is not "ready", not topo order, so return false
        return false;
      }
      for (edge& e: vertices[order[i]].outgoing){// decrease indegree of all outgoing neighbors of that vertex in the order
        inDegree[e.vertex_id]--;
      }
    }
  ...
  void pathFinder(int target, vector<int> &path, vector<string> &paths){
    int v;
    string p = "";

    for (edge &e : vertices[path.back()].outgoing){
